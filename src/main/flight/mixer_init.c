...
void mixerInitProfile(void)
{
    ...
    if (currentPidProfile->vbat_sag_compensation > 0 && !RPM_LIMIT_ACTIVE) {
        ...
        const uint16_t warningCellVoltage = batteryConfig()->isLiionModeActive ? batteryConfig()->vbatwarningcellvoltageLiion : batteryConfig()->vbatwarningcellvoltage;
        mixerRuntime.vbatRangeToCompensate = mixerRuntime.vbatFull - warningCellVoltage;
